...
import { parseFromUrl, parseFromText, describeResult } from "../lib/parseJob.js";
...
export default function ApplicationForm({ initial, files, onSave, onClose, onDelete }) {
  ...
  const readLink = async () => {
    ...
    try {
      ...
      if (message.suggestPaste) setShowPaste(true);
      setNote({ kind: message.kind, text: message.text });
    } catch (err) {
      setShowPaste(true);
